...
def score_population(population):
  ...
  for i in range(len(population)):
    ...
    scores[i][4]=float(metrics['preference_average'])
  return scores
...
def main_NSGA2():
  ...
  args = ['C:/Program Files/R/R-4.0.3/bin/Rscript', '--vanilla', 'PostProcessingV2.R','outputs/'+output_date+'/'+log]
  subprocess.call(args,shell=True,stdout=subprocess.PIPE)
  with open('utilities/'+log+'_hash_with_policy.json', 'w', encoding='utf-8') as f:
...
def calculate_fitness2(solution):
  ...
  if solution_hash in hash_dictionary:
    ...
    waiting_time_average=100/len(set(solution))
    flow_time_average=200-waiting_time_average
    cost_average=rn.randint(1,200)
    workload_average=1-waiting_time_average/flow_time_average
    ...
    hash_dictionary[solution_hash]=[cost_average,flow_time_average,waiting_time_average,workload_average]
...
def calculate_fitness(solution):
  ...
  if solution_hash in hash_dictionary:
      ...
      waiting_time_average=hash_dictionary[solution_hash][2]
      workload_average=hash_dictionary[solution_hash][3]
      preference_average=hash_dictionary[solution_hash][4]
      results_dictionary={
          "waiting_time_average":waiting_time_average,
          "flow_time_average":flow_time_average,
          "cost_average":cost_average,
          "workload_average":workload_average,
          "preference_average":preference_average
       }
  else:
    #   print('SIMULATION STARTED at:'+str(starting_time))
       ...
       activities_tree.write('samples/'+log+'ScyllaSimuConfig.xml')
      
       args = ['java', '-jar', 'Scylla_V6.jar',"--config="+'samples/'+log+"ScyllaGlobalConfig.xml","--bpmn="+'samples/'+log+".bpmn","--sim="+'samples/'+log+"ScyllaSimuConfig.xml","--output=scylla_results_","--enable-bps-logging"]
       subprocess.call(args,shell=True,stdout=subprocess.PIPE)
     ...
     #  print('SIMULATION FINISHED, duration='+str(duration))
       ...
       preference_average=calculate_preference(solution)
       #print('AVERAGE')
       ...
       hash_dictionary[solution_hash]=[cost_average,flow_time_average,waiting_time_average,workload_average,preference_average]
       os.remove(output_file_name+"ScyllaGlobalConfig_resourceutilization.xml")
       os.remove(output_file_name+".xes")
       os.remove(output_file_name+"ScyllaGlobalConfig_batchactivitystats.txt")
       os.rmdir("scylla_results_")
...
logs=['ConsultaDataMining201618','Production','PurchasingExample']
  
for l in logs :
                    log=l
                    #Para minimizar poner -1, para maximizxar 1y pa no hacer nada 0
                    nonrepeated_resources=False
                    cost=-1
                    workload=1
                    flow_time=-1
                    waiting_time=-1
                    metrics_number=5
                    #abs(cost)+abs(workload)+abs(flow_time)+abs(waiting_time)
                    json_file=open('samples/'+log+'_canon.json')
                    ...
                    bpmn_tree = ET.parse('samples/'+log+'.bpmn')
                    ...
                    with open('samples/'+log+'_modified.json', 'w', encoding='utf-8') as f:
                        json.dump(canon_model, f, ensure_ascii=False, indent=4)
                    
                    xml_writer_scylla.print_parameters('samples/'+log+'Scylla',canon_model)
                    repair_xml('samples/'+log+'ScyllaGlobalConfig.xml')
                    
                    
    

                    #pmx=np.loadtxt(open("NSGA2_RESOURCES_PMX.csv"),delimiter=",", skiprows=1)
                    resources_tree = ET.parse('samples/'+log+'ScyllaGlobalConfig.xml')
                    ...
                    try:
                     with open('utilities/'+log+'_hash_with_policy.json') as json_file:
                    ...
                    activities_tree = ET.parse('samples/'+log+'ScyllaSimuConfig.xml')
                    ...
                    args = ['C:/Program Files/R/R-4.0.3/bin/Rscript', '--vanilla', 'PreProcessing.R','samples/'+log]
                    subprocess.call(args,shell=True,stdout=subprocess.PIPE)
                    incidence_matrix = pd.read_csv('samples/'+log+'_incidenceMatrix.csv', index_col=0)
                    output_date=datetime.datetime.now().strftime("%B%d%Y%H%M%S")

                    baseline_output=[0,0,0,0,0]
